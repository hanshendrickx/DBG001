...
## 📖 Installation
my_blog_app can be installed via Pip, Pipenv, or Docker. To start, clone the repo to your local computer and change into the proper directory.

```
$ git clone https://github.com/wsvincent/my_blog_app.git
$ cd my_blog_app
```
...
To use Docker with PostgreSQL as the database update the `DATABASES` section of `my_blog/settings.py` to reflect the following:
...
The `INTERNAL_IPS` configuration in `my_blog/settings.py` must be also be updated:
